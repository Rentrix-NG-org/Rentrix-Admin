...
const WithdrawalRequestInfo = () => {
  ...
  const { getWithdrawalRequestDetails } = UserService();
  ...
  useEffect(() => {
    async function fetchLog() {
      const response = await getWithdrawalRequestDetails(param?.logId || "");
      if (response.success) {
          ...
          <Typography
            sx={{
              color:
                ...
                log?.status?.toLowerCase() === "processed"
...
export default WithdrawalRequestInfo;
